...
+ (void)load
{
    ...
    // === KEY AUTH SYSTEM ACTIVATED ===
    // User must enter a valid license key before the mod menu can be used
    // Keys are device-specific and generated by the admin
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if ([[KeyAuth sharedInstance] isAuthenticated]) {
            ...
            [[KeyAuth sharedInstance] showLoginWithCompletion:^(BOOL success) {
                if (success) {
                    [extraInfo initUIKitGesture];
                    [[NSNotificationCenter defaultCenter] addObserver:self
                                                             selector:@selector(menuDidClose)
